...
if __name__ == '__main__':
    ...
    for GSM8k_file in GSM8k_files:
        ...
        print(f'{variant_name}: Acc: {accuracy * 100:.2f}%, Confidence Interval: ({lower_bound},{upper_bound})')

    print('GSM Plus metrics:')
    for GSM_plus_file in GSM_plus_files:
        if GSM_plus_file == 'GSM-plus-zero_shot.jsonl':
            continue
    ...
    # Calculate for GSM-plus-zero-shot separately
    with open('evaluation_result/GSM-plus-zero_shot.jsonl', 'r', encoding='utf-8') as f:
        true = 0
        all = 0
        lines = f.readlines()
        for line in lines:
            if 'binary_solution' in line:
                parts = line.split('binary_solution', 1)
        ...
        accuracy = true / all
        # Compute confidence interval
        epsilon = 1.96 * math.sqrt( (accuracy) * (1 - accuracy) / all)
        lower_bound = accuracy - epsilon
        upper_bound = accuracy + epsilon
        print(f'zero-shot: Acc: {accuracy * 100:.2f}%, Confidence Interval: ({lower_bound},{upper_bound})')
